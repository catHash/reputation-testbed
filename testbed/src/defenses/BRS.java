/**class BRS: 
 * Inherits methods and attributes from the Defense class. The BRS utilizes a statistical filtering
 * approach to exclude unfair ratings. In order to identify if the advisor is being unfair or not, 
 * hypothesis testing is required to determine. The process begins by calculating the reputation 
 * of a seller given a set of honest advisors. If the reputation score calculated falls in the rejection region 
 * of the beta distribution of advisors’ ratings, that particular advisor will be omitted from 
 * the set of honest advisors since his ratings is unfair and therefore unreliable to be considered. The rejection region is lower-bounded by the q quantile and upper-bounded by the (1-q) quantile of the beta distribution of advisors’ ratings. 
 * (q takes the value from 0.00 to 0.50 and it has a default value of 0.01)
 */
...
public class BRS extends Defense{
    ...
    // the reputation of the seller is calculated based on the approach taken by the model.
	...
	//choose seller based on the reputation computed as per the approach taken by the defense model.
