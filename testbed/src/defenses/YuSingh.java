...
public class YuSingh extends Defense {
	/**
	 *  * Trust function from (Yu and Singh, 2000)
	 * a social mechanism of reputation management in electronic communities
	 * 
	 * where α >= 0, β <= 0:
	 * Cooperation
	 * T(t) > 0		T(t+1) = Tt+α(1-Tt) 
	 * T(t) < 0		T(t+1) = (Tt+α)/(1-min{|Tt|,|α|}) 
	 * T(t) = 0		T(t+1) = α
	 * Defection
	 * T(t) > 0		T(t+1) = (Tt+β)/(1-min{|T|,|β|}) 
	 * T(t) < 0		T(t+1) = Tt+β(1+Tt) 
	 * T(t) = 0		T(t+1) = β
	 * 
	 * 
	 * α = 0.05, β = -0.3
	 * α = 0.05,0.1,0.2 β = -0.1,-0.2,-0.3
	 */
	public double trustFunction(double trustValue, boolean isPositiveRating){
		...
		if (isPositiveRating == true){
			if(trustValue>0){
				return (trustValue + (alpha * ( 1 - trustValue)));
			}
			else if (trustValue<0){
				return ((trustValue + alpha)/(1-Math.min(trustValue,alpha)));
			}
			else{
				return alpha;
			}	
			...
			if(trustValue>0){
				return ((trustValue + beta)/(1-Math.min(trustValue,beta)));
			}
			else if (trustValue<0){
				return ((trustValue)+(beta*(1+trustValue)));
			}
			else{
				return beta;
			}
