...
public class BetaDistribution extends Distribution{
	...
	public void setParameters(double a, double b) {
		this.a = a;
		this.b = b;
		c = Distribution.logGamma(a + b) - Distribution.logGamma(a)
				- Distribution.logGamma(b);
		double lower;
		if (a < (double) 1) {
			lower = 0.001D;
		} else {
	...
	public double density(double x) {
		...
		if ((x == (double) 1) & (b > (double) 1)) {
			...
			return Math.exp(c + (a - (double) 1) * Math.log(x)
					+ (b - (double) 1) * Math.log((double) 1 - x));
		}
	}
	...
	public double CDF(double x) {
		return Distribution.betaCDF(x, a, b);
	}
